...
export default function MisAnalystsPage() {
  const data = [
  ...
  return (
    <div className="xl:overflow-hidden h-full flex flex-col">
      <DashboardHead>
        <h2 className="font-medium">MIS Analysts</h2>
      ...
      <div className="p-8 pb-16 overflow-y-auto">
        <div className="mb-4 flex">
          ...
          <div className="flex gap-2 items-center">
            <Link href="/mis-analysts/heirarchy">
              <a className="OutlineButton uppercase">Team Heirarchy</a>
            </Link>
            <button className="Button uppercase">Add new MIS</button>
          </div>
        </div>
              ...
              {data.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="border-x-0 py-2">
                    <Link href={`/mis-analysts/details/${row.id}`}>
...
MisAnalystsPage.getLayout = function getLayout(page: ReactElement) {
