import { useGetBlogs } from '@corpcare/shared/api';
import { ArticlesCard } from '../../components/shared/ArticlesCard';
...
const Articles = ({ global }) => {
  ...
  const { data } = useGetBlogs(pageNumber);
  return (
      ...
      <div className="container mx-auto lg:px-10 px-5 py-16 lg:py-[80px]">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 place-items-center">
          {data?.pageData?.map(({ attributes, id }, index) => (
            <ArticlesCard data={attributes} key={id + index} />
          ))}
