...
export function IndividualSignupForm() {
  ...
  const {
    ...
    resolver: yupResolver(individualSignupSchema),
  });

  const { mutate, isLoading } = useSignupIndividual();

  const handleSignup = async (data: IndividualSignupFields) => {
    mutate(data, {
      onSuccess() {
        toast.custom((t) => (
          <CustomToast
