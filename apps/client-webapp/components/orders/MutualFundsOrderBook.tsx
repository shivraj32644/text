...
import { MutualFundOrderCard } from './MutualFundOrderCard';

export function MutualFundsOrderBook({
  title,
  orders,
  setFiltersOpen,
  isLoading,
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-10">
        {orders?.map((order) => (
          <MutualFundOrderCard key={order?.order_number} {...order} />
        ))}
      </div>
    </div>
  );
