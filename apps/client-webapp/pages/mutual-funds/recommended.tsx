...
export default function MutualFundRecommended() {
  ...
  const { data: recommendedMutualFunds, isLoading } = useGetMutualSchema(
    currentPage,
    searchTerm,
    options,
    amc,
  ...
  return (
    <div className="xl:overflow-hidden h-full flex flex-col">
      ...
      <div className="p-4 lg:p-8 pb-10 lg:pb-16 overflow-y-auto">
        <div className="relative">
          <img
            className="w-full h-40 lg:h-full max-h-64 object-cover rounded-lg"
            src="/mutual-funds-recommended-banner.png"
            alt="CorpCare - Mutual Funds Recommended"
          />
          <h1 className="absolute lg:bottom-8 bottom-2 left-2 lg:left-8 text-white text-lg lg:text-3xl max-w-[481px]">
            Explore CorpCare recommended funds with an excellent track record of
            providing higher returns.
          </h1>
          <div className="bg-brand w-10 h-10 rounded absolute right-8 bottom-8 items-center justify-center hidden lg:inline-flex">
            <QuestionMarkIcon className="text-white w-7 h-7" />
          </div>
        </div>
        <form
            ...
            <Tab.Panels>
              <Tab.Panel>
                <div className="bg-white rounded-lg border overflow-x-auto hidden lg:block">
                  <table className="border-hidden">
                    ...
                    <tbody>
                      {recommendedMutualFunds?.items?.length === 0 ? (
                        ...
                        recommendedMutualFunds?.items?.map((item) => (
                                ...
                                >
                                  {col?.name === 'plan_name' ? (
                                    ...
                                    <div className="flex flex-shrink-0">
                                      <a className="OutlineButton inline-block p-2 opacity-25 pointer-events-none">
                                        <BrandPlusAddIcon />
                                      </a>
                  ...
                  {!isLoading && !!recommendedMutualFunds?.count && (
                    <ReactPaginate
                      forcePage={currentPage - 1}
                      onPageChange={({ selected }) =>
                        setCurrentPage(selected + 1)
                      }
                      pageCount={Math.ceil(recommendedMutualFunds?.count / 10)}
                ...
                <section className="lg:hidden">
                  {recommendedMutualFunds?.items.length !== 0 ? (
                    <>
                      {recommendedMutualFunds?.items?.map((mf, idx) => (
                        <div
                          key={idx}
                          className={classNames(
                            ...
                            {({ open }) => (
                              <>
                                ...
                                <Disclosure.Panel className="flex flex-col divide-y">
                                  ...
                                  <section className="flex gap-5 py-3 px-5 items-center justify-center">
                      ...
                      {!isLoading && !!recommendedMutualFunds?.count && (
                        <ReactPaginate
                          ...
                          pageCount={Math.ceil(
                            recommendedMutualFunds?.count / 10
                          )}
                ...
                <Dialog.Panel className="ml-auto relative max-w-xs w-full h-full bg-white shadow-xl flex flex-col">
                  ...
                  <div className="flex-grow overflow-y-auto">
                    ...
                    <Disclosure
                      as="div"
                      className={classNames(
                        'px-4 py-6 border-t border-brandLight'
                      )}
                    >
                      {({ open }) => (
                        <>
...
MutualFundRecommended.getLayout = function getLayout(page: ReactElement) {
